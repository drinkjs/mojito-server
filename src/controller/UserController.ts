...
import { BaseController, Body, Controller, Get, Query, RouterContext, Validation, Headers, AppError } from "ngulf";
...
export default class UserController extends BaseController {
  ...
  async add(@Query(new Validation()) dto: LoginDto, ctx: RouterContext) {
    let loginRel: { token: string, name: string, id: string, avatarUrl?: string } | null = null;
    let user = null;
    if (dto.from === "github") {
  ...
  async githubAuth(code: string) {
    ...
    if (response.status !== 200) {
      AppError.assert(`https://github.com/login/oauth/access_token ${response.statusText}`)
    }
    ...
    if (response && response.status === 200) {
      ...
      if (userInfo && userInfo.status === 200) {
        ...
        if (name && id) {
          return await this.service.add({ from: "github", name, email, authUserId: id, avatarUrl: avatar_url })
        }
      }
    }
  }
  ...
  async giteeAuth(code: string, redirectUri?: string) {
    ...
    const response = await axios.post(`https://gitee.com/oauth/token?grant_type=authorization_code&code=${code}&client_id=${client_id}&redirect_uri=${redirectUri}`,
      { client_secret },
      {
        headers: {
          Accept: "application/json"
    ...
    if (response && response.status === 200) {
      ...
      if (userInfo && userInfo.status === 200) {
        const { name, email, avatar_url, id } = userInfo.data;
        if (name && id) {
          return await this.service.add({ from: "gitee", name, email, authUserId: id, avatarUrl: avatar_url })
        }
      }
    }
  }
