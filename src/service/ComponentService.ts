...
export default class ComponentService extends BaseService {
	@Mongo(ComponentTypeEntity)
	private typeModel!: MongoModel<ComponentTypeEntity>;

	@Mongo(ComponentEntity)
	private model!: MongoModel<ComponentEntity>;
	...
	async addType(data: ComponentTypeDto, userId: string) {
		const rel = await this.typeModel
			.findOne({
				$or: [
					{ name: data.name, pid: data.pid, userId },
					{ origin: 999, name: data.name, pid: data.pid },
				],
			})
			.exec();
		if (rel) {
	...
	async findAllLibs(type: string, userId: string) {
		const rel = await this.model
			.find({
				$or: [
					{ type, userId, deleteAt: null },
					{ origin: 999, type, deleteAt: null },
				],
			})
			.sort({ _id: -1 })
			.exec();
	...
	async findById(id: string | string[], userId: string) {
		return typeof id === "string"
			? this.toObject(
					await this.model.findOne({ _id: id, userId, deleteAt: null }).exec()
			  )
	...
	async updateLib(data: ComponentDto, userId: string) {
		...
		return await this.model.updateOne(
			{ _id: data.id, userId },
			{ ...data, updateAt: new Date(), origin: 1 },
			{ omitUndefined: true }
		);
	}
	...
	async updateIconFont(
		...
		if (data.url) {
			return await this.iconModel.updateOne(
				{ _id: data.id, userId },
				{
					url: data.url,
					type: data.type,
